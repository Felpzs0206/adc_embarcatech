...
void button_callback(uint gpio, uint32_t events) {
    ...
    if (gpio == BUTTON_JOYSTICK_PIN && (now - last_button_a_time) > debounce_time_ms) {
...
int main()
{
    ...
    gpio_init(BUTTON_JOYSTICK_PIN); // Inicializa o pino do botão do joystick
    gpio_set_dir(BUTTON_JOYSTICK_PIN, GPIO_IN); // Configura o pino do botão do joystick como entrada
    ...
    gpio_set_irq_enabled_with_callback(BUTTON_JOYSTICK_PIN, GPIO_IRQ_EDGE_FALL, true, &button_callback); // Habilita a interrupção para o botão do joystick
